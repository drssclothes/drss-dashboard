...
function todayMoscow() {
  // Сервер GitHub Actions работает в UTC — переводим в МСК (UTC+3)
  const now = new Date();
...
function getCurrentWeekRange(dateStr) {
  // Текущая (ещё идущая) неделя пн-вс, содержащая dateStr
...
// ── JWT для Google Service Account (без внешних библиотек) ──
...
// ── Чтение листа LOG: столбцы C (артикул), D (старая цена), E (новая цена), F (дата) ──
async function fetchPriceChangesToday(accessToken, todayStr) {
  ...
  // Парсим дату из столбца F в разных возможных форматах (DD.MM.YYYY или YYYY-MM-DD)
  ...
  // Последнее изменение цены за сегодня по каждому артикулу (если их несколько за день — берём последнее)
  const priceByVc = {};
  rows.forEach(row => {
...
// ── Supabase: читаем текущую неделю, обновляем byDay[today].price, сохраняем ──
